...
def clear_release_quarantine(
    release: Release, request, flash: bool = True
) -> None:
    """
    ...
    release.lifecycle_status_note = (
        f"Quarantine cleared by {request.user.username}."
    )
    ...
    if flash:
        request.session.flash(
            f"Release {release.version} of {release.project.name} "
            "quarantine cleared.",
            queue="success",
        )
